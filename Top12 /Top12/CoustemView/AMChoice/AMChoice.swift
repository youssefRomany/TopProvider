...
class AMChoice: UIView,UITableViewDelegate,UITableViewDataSource {
    ...
    override func draw(_ rect: CGRect) {
        ...
        //        let backgroundView = UIView(); backgroundView.backgroundColor = backColor
    ...
    final func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //cell.imgArrow.image = arrowImage?.maskWithColor(color: PRIMARY_LIGHT)
        
        cell.lblTitle.text = item.title
        cell.lblTitle.font = font ?? cell.lblTitle.font
        cell.lblTitle.font = UIFont(name: "JF-Flat-regular", size: 15)
        cell.lblTitle.textColor = #colorLiteral(red: 0.1298420429, green: 0.1298461258, blue: 0.1298439503, alpha: 1)
    ...
    func getSelectedItemsJoined(separator: String = ",") -> String {
        ...
        let selectedItem = data.filter { (item) -> Bool in
            return item.isSelected == true
            }.map{String($0.oilId)}.joined(separator: separator)
        
        return selectedItem
    }
    func getSelectedItemsTitleJoined(separator: String) -> String {
